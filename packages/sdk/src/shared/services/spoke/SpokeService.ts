...
export class SpokeService {
  ...
  public readonly aleoSpokeService: AleoSpokeService;
  ...
  public constructor({ config, hubProvider }: SpokeServiceConstructorParams) {
    ...
    this.aleoSpokeService = new AleoSpokeService(this.config);
  }
  ...
  public getSpokeService<C extends SpokeChainKey>(chainKey: C): GetSpokeServiceType<C> {
    ...
    switch (chainType) {
      ...
      case 'ALEO': {
        return this.aleoSpokeService satisfies GetSpokeServiceType<AleoChainKey> as GetSpokeServiceType<C>;
      }
      default: {
  ...
  public async estimateGas<C extends SpokeChainKey>(
    ...
    try {
      ...
      switch (chainType) {
        ...
        case 'ALEO': {
          const value = (await this.aleoSpokeService.estimateGas(
            params as EstimateGasParams<AleoChainKey>,
          )) as GetEstimateGasReturnType<C>;
          return { ok: true, value };
        }
        default: {
    ...
    try {
      ...
      switch (chainType) {
        ...
        case 'ALEO': {
          const verify = await this.verifyDepositSimulation(params);
          if (!verify.ok) return verify;
          const value = (await this.aleoSpokeService.deposit(
  ...
  public async getDeposit<C extends SpokeChainKey>(params: GetDepositParams<C>): Promise<Result<bigint>> {
    try {
      ...
      switch (chainType) {
        ...
        case 'ALEO': {
          const value = await this.aleoSpokeService.getDeposit(params as GetDepositParams<AleoChainKey>);
          return { ok: true, value };
        }
        default: {
          const exhaustiveCheck: never = chainType;
    ...
    try {
      ...
      switch (chainType) {
        ...
        case 'ALEO': {
          const verify = await this.verifySimulation(params);
          if (!verify.ok) return verify;
          const value = (await this.aleoSpokeService.sendMessage(
  ...
  public async waitForTxReceipt<C extends SpokeChainKey = SpokeChainKey>(
    ...
    try {
      ...
      switch (chainType) {
        ...
        case 'ALEO': {
          return (await this.aleoSpokeService.waitForTransactionReceipt(
